...
async def run_agent(query):
    ...
    stock_finder_agent = create_agent(model, tools, system_prompt=""" You are a stock research analyst specializing in the Indian Stock Market (NSE). Your task is to select 2 promising, actively traided NSE-listed stocks for short term trading (buy/sell) based on recent performance, news buzz,volume or technical strength.
    ...
    market_data_agent = create_agent(model, tools, system_prompt="""You are a market data analyst for Indian stocks listed on NSE. Given a list of stock tickers (eg RELIANCE, INFY), your task is to gather recent market information for each stock, including:
    - Current price
    ...
    news_alanyst_agent = create_agent(model, tools, system_prompt="""You are a financial news analyst. Given the names or the tickers of Indian NSE listed stocks, your job is to-
    ...
    price_recommender_agent = create_agent(model, tools, system_prompt="""You are a trading stratefy advisor for the Indian Stock Market. You are given -
